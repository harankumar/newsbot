...
const SITEMAP_URL_BASE = "http://spiderbites.nytimes.com/sitemaps/www.nytimes.com/sitemap_2017_";
...
function readNYTarticle(entry, callback){
  setTimeout(function(){
    ...
    scrape(entry.loc, {
      author: {
        listItem: ".byline-author"
      },
      title: "[itemprop=headline]",
      body: {
        listItem: ".story-body-text.story-content"
      }
...
//            .pipe(zlib.createGunzip()))
//  .then(parseSitemap)
getStream(request(SITEMAP_URL_BASE + 11 + ".xml.gz")
            .pipe(zlib.createGunzip()))
  .then(parseSitemap)
getStream(request(SITEMAP_URL_BASE + 12 + ".xml.gz")
            .pipe(zlib.createGunzip()))
  .then(parseSitemap)
